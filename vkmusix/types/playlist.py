...
class Playlist(Base):
    ...
    async def getTracks(self, isLarge: bool = False) -> Union[List[Track], None]:
        ...
        :param isLarge: флаг, указывающий, содержит ли плейлист или альбом более 1000 треков. По умолчанию ``False``. Если ``True``, будет получена ограниченная информация о всех треках, если ``False`` — только первые 1000 треков, но с полной информацией. Установите ``None``, чтобы библиотека определила автоматически. Игнорируется для приватных плейлистов. (``bool``, optional)
